<template>
    <div>
      ...
      <div class="card" >
        <el-input v-model="data.key" clearable style="width: 200px;margin-right: 10px" placeholder="请输入用户名或姓名"/>
      ...
      <div class="card" style="margin-top: 5px" >
        <el-table :data="data.tableData"
          ...
          <el-table-column prop="protypename" label="口味偏好" />
          <el-table-column prop="jktype" label="健康状况" />
          <el-table-column prop="mbtype" label="饮食目标" />
          <el-table-column prop="status" label="状态" >
            <template #default="scope">
              {{scope.row.status}}
              <el-button type="danger" v-if="scope.row.status==='正常'"  size="small" @click="upateEdit(scope.row.id,'封禁')">封禁</el-button>
              <el-button type="success" v-else size="small" @click="upateEdit(scope.row.id,'正常')">解除</el-button>
            </template>
          </el-table-column>
      ...
      <el-dialog :title="data.title" width="60%" v-model="data.formVisible" :close-on-click-modal="false" destroy-on-close>
        <el-form :model="data.form" label-width="100px" ref="formRef" :rules="data.rules" style="padding-right: 50px;margin-top: 20px;">
          ...
          <el-form-item v-show="data.title==='用户信息新增'" label="密码" prop="password">
...
//信息列表
const load =()=>{
  request.post('memberList', qs.stringify({
    key:data.key,
...
// 封禁或解除账号
const upateEdit = (id,status) => {
  request.post('memberEdit', qs.stringify({
    id:id,
    status:status,
  })).then(res => {
    if (res.code === '200') {
...
const Delete = (id) => {
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗?', { type: 'warning' }).then(res => {
    request.post('memberDel?id=' + id).then(res => {
      if (res.code === '200') {
        load()
        ElMessage.success('操作成功')
      } else {
...
const delAll=()=>{
  if(data.ids.length === 0){
    ...
    ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗?', { type: 'warning' }).then(res => {
      request.post('memberDeleteAll',qs.stringify({
        vals:data.ids
      })).then(res => {
        if (res.code === '200') {
          load()
