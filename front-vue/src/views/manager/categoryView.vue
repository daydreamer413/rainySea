...
//信息列表
const load =()=>{
  request.post('categoryList', qs.stringify({
    key:data.key,
...
const save = () => {
  formRef.value.validate((valid=>{
    if (valid){
      request.post('categoryAdd', qs.stringify({
        id:data.form.id,
        name:data.form.name,
      })).then(res => {
        if (res.code === '200') {
...
const Delete = (id) => {
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗?', { type: 'warning' }).then(res => {
    request.post('categoryDel?id=' + id).then(res => {
      if (res.code === '200') {
        load()
        ElMessage.success('操作成功')
      } else {
...
const delAll=()=>{
  if(data.ids.length === 0){
    ...
    ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗?', { type: 'warning' }).then(res => {
      ...
      request.post('categoryDeleteAll',qs.stringify({
        vals:data.ids
      })).then(res => {
        if (res.code === '200') {
          load()
